...
def create_card(id, voting, rhetoric, donation):
    ...
    cur.execute(
        f"SELECT name, district, title, img_link FROM legislators WHERE id= '{id}';")
    ...
    scaled_portrait = portrait.resize(
        (config["portrait_scale"][0], scaled_height), Image.ANTIALIAS)
    ...
    name_fnt = ImageFont.truetype(
        config["font"]["name"], config["font_size"]["m"])
    district_fnt = ImageFont.truetype(
        config["font"]["district"], config["font_size"]["s"])
    updated_fnt = ImageFont.truetype(
        config["font"]["district"], config["font_size"]["xs"])
    grade_fnt = ImageFont.truetype(
        config["font"]["bold"], config["font_size"]["l"])

    # Draw grades
    d.text(config["location"]["voting"], voting, font=grade_fnt,
           fill=tuple(config["color"]["font_1"]))
    d.text(config["location"]["rhetoric"], rhetoric,
           font=grade_fnt, fill=tuple(config["color"]["font_1"]))
    d.text(config["location"]["donation"], donation,
           font=grade_fnt, fill=tuple(config["color"]["font_1"]))
    ...
    d.text(config["location"]["district"], district_str,
           font=district_fnt, fill=tuple(config["color"]["font_1"]))
    ...
    if len(name) <= 30:
        ...
        if len(wrapped_name) == 1:
            ...
            d.text((553, 112), line_1, font=name_fnt,
                   fill=tuple(config["color"]["font_1"]))

        # Draw Two lines
        elif len(wrapped_name) == 2:
                line_1, line_2 = wrapped_name
                d.text((553, 89), line_1, font=name_fnt,
                       fill=tuple(config["color"]["font_1"]))
                d.text((553, 135), line_2, font=name_fnt,
                       fill=tuple(config["color"]["font_1"]))
    elif len(name) > 30:
        raise ValueError(
            "Name is too long. Shorten the name or ask project manager to patch program")

    # Draw updated tag
    updated_location = config["location"]["updated"]
    updated_location[0] -= d.textsize(date, font=updated_fnt)[0]
    d.text(updated_location, date, font=updated_fnt,
           fill=(tuple(config["color"]['font_2'])))
...
def batch_create(destination):
    cur.execute(
        f"SELECT id, voting, rhetoric, donations FROM grades WHERE assembly LIKE '{assembly}%'")
    for row in cur.fetchall():
        cur2 = db.cursor()
        cur2.execute(
            f"SELECT last, first, house, district FROM legislators WHERE id='{row[0]}'")
...
if __name__ == '__main__':
    cur.execute(
        "SELECT id, voting, rhetoric, donations FROM grades WHERE id ='71repmichaelson jenet30' OR id = '71repwinkler34'")
    for row in cur.fetchall():
